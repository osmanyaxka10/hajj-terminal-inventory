...
import { emptyMap, movementSeries } from "./engine";
...
export async function recordForecastSnapshot(input: {
  ...
  for (const r of input.recommendations) {
    ...
    const payload = {
      ...
      forecast_date: input.orderFor,
      predicted_movement: r.forecast,
      recommended_order_qty: r.recommended,
      approved_order_qty: Math.max(0, Number(input.approved[r.productId] || 0)),
      confidence: r.confidence,
      notes: r.explanation
    };
