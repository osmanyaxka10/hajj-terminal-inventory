...
import { loadRecords, saveRecords, exportRecords } from "@/lib/storage";
...
export default function InventoryApp() {
  ...
  async function saveOperation() {
    if (!userEmail) {
      alert("Sign in to save receiving, transfers, waste or adjustments.");
      return;
    }
    setSyncing(true);
    try {
  ...
  async function approveOrder() {
    if (!userEmail) {
      alert("Sign in before approving a cloud order.");
      return;
    }
    setSyncing(true);
    try {
      ...
      <>
        <header className="top">
          <div className="top-inner">
            ...
            <span className="badge warn">Cloud sign-in needed</span>
          </div>
        </header>
        <main className="shell">
          <AuthPanel onSignedIn={refreshCloud} />
          <div className="notice">
            Offline/local inventory still works. Cloud operations, audit history and
            cross-device sync require sign-in.
          </div>
        </main>
      </>
    );
